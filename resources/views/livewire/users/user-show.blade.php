<div>
    <div class="relative mb-6 w-full">
        ...
        <flux:subheading size="lg" class="mb-6">{{ __('this page is for show user') }}
        </flux:subheading>
                ...
                <div class="flex items-center gap-6">
                    <div class="flex-shrink-0">
                        @if (!empty($user->avatar))
                            <img src="{{ strpos($user->avatar, 'http') === 0 ? $user->avatar : asset('storage/' . $user->avatar) }}"
                                alt="{{ $user->name }}" class="h-20 w-20 rounded-full object-cover" />
                        @else
                            <div
                    ...
                    <div>
                        ...
                        @if (isset($user->created_at))
                            <p class="text-xs text-gray-400 mt-2">Joined: {{ $user->created_at->format('d M Y') }}</p>
                        @endif
                    </div>
                </div>
            </div>
