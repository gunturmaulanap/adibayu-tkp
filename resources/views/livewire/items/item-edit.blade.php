<div>
    <div class="relative mb-6 w-full">
        <flux:heading size="xl" level="1">{{ __('Edit Item') }}</flux:heading>
        <flux:subheading size="lg" class="mb-6">{{ __('Form for edit your item') }}
        </flux:subheading>
        ...
        <div class="p-3">
            ...
            <div class="w-150 mt-6">
                <form wire:submit="submit" class="mt-3 space-y-6">
                    ...
                    <!-- Preview: show temporary upload preview if user selected a new file, otherwise show existing image or placeholder -->
                    <div class="mt-2">
                        @if (isset($image) && method_exists($image, 'temporaryUrl'))
                            <img src="{{ $image->temporaryUrl() }}" alt="Preview"
                                class="h-24 w-24 object-cover rounded" />
                        @elseif(!empty($existingImage))
                            <img src="{{ strpos($existingImage, 'http') === 0 ? $existingImage : asset('storage/' . $existingImage) }}"
                                alt="Existing image" class="h-24 w-24 object-cover rounded" />
                        @else
                            <div
