    ...
    <flux:sidebar sticky stashable class="border-e border-zinc-200 bg-zinc-50 dark:border-zinc-700 dark:bg-zinc-900">
        ...
        <flux:navlist variant="outline">
            <flux:navlist.group :heading="__('Platform')" class="grid">
                ...
                @endif
                @if (auth()->user()->can('item.view') ||
                        auth()->user()->can('item.create') ||
                        auth()->user()->can('item.edit') ||
                        auth()->user()->can('item.delete'))
                    <flux:navlist.item icon="list-bullet" :href="route('items.index')"
                ...
                @if (auth()->user()->can('sale.view') ||
                    ...
                    <flux:navlist.item icon="currency-dollar" :href="route('sales.index')"
                        :current="request()->routeIs('sales.index')" wire:navigate>{{ __('Sales') }}
                    </flux:navlist.item>
                @endif
                @if (auth()->user()->can('payment.view') ||
                        auth()->user()->can('payment.create') ||
                        auth()->user()->can('payment.edit') ||
                        auth()->user()->can('payment.delete'))
                    <flux:navlist.item icon="credit-card" :href="route('payments.index')"
                        :current="request()->routeIs('payments.index')" wire:navigate>{{ __('Payments') }}
                    </flux:navlist.item>
                @endif

                {{-- <flux:navlist.item icon="currency-dollar" :href="route('transactions.index')"
